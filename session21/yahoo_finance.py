...
def parse_html(html):
    ...
    # return stock_list
    for stock_row in stock_table.find_all('tr'):
        stock_name = stock_row.find('td', {'class': 'data-col1'}).text.strip()
        symbol = stock_row.find('td', {'class': 'data-col2'}).text.strip()
...
def main():
    ...
    stock_list = parse_html(html)
    
    with open('yahoo_finance_trending_tickers.csv', 'w', newline='') as csvfile:
